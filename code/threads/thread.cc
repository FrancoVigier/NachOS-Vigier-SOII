...
Thread::Thread(const char *threadName, bool joinableThread, unsigned int priorityThread)
{
    name     = threadName;
    ...
    stack    = nullptr;
    status   = JUST_CREATED;
    joinable = joinableThread;
    if(joinable) {
	    threadChannel = new Channel("Canal del thread");
	}
	inheritedPriorities = nullptr;
#ifdef USER_PROGRAM
    space    = nullptr;
    opFD = new Table <OpenFile*>;  //EJ 2. P3
    //stdIn
    opFD ->Add(NULL);
    //stdOut
    opFD ->Add(NULL);
    //id Thread
    tId = threadUPS ->Add(this);
#endif
///
#ifdef FILESYS
...
#ifdef USER_PROGRAM
	//threadUPS ->Remove(tId);
	if(space != nullptr)
	    delete space;

///**
    for(int i = 0; opFD->HasKey(i); ++i) {
        delete opFD->Get(i);
    }
///
	delete opFD;
...
void
Thread::SetPreAssignedQueue(unsigned int index) {
	preassignedQueue = index;
}

unsigned int
Thread::GetPreAssignedQueue() const {
	return preassignedQueue;
}
...
{
	int p = priority;
	for (PriorityInheritanceList* i = inheritedPriorities; i != nullptr; i = i->next) {
		if(i->lock != nullptr && p < i->priority) {
			p = i->priority;
		}
	}
	return p;
...
{   interrupt->SetLevel(INT_OFF);
  ...
  //      interrupt->Halt();
    ...
    ///
 //      threadUPS->Remove(tId); // Si muere el Main muere todo
 //   if(threadUPS->IsEmpty()){
 //       tId = threadUPS->Add(this);
 //       interrupt->Halt();
 //   }
 //   tId = threadUPS->Add(this);
    ...
    DEBUG('t', "Finishing thread \"%s\", with return: %d\n", GetName(), returnValue);

	if(joinable) {
	    //Aviso por el canal del Thread que este ya termino con el valor que paso por var
        threadChannel->Send(returnValue);
	}
		threadToBeDestroyed = currentThread;
		//This will invoke sleep until the thread is joined

		Sleep(consoleON);  // Invokes `SWITCH`.
...
Thread::Yield()
{
	...
	SetPreAssignedQueue(1); //Descend if work is slow
...
Thread::Sleep(bool consoleON)
{
	...
	SetPreAssignedQueue(0); //Ascend if work is fast

    Thread *nextThread;
    status = BLOCKED;
    while ((nextThread = scheduler->FindNextToRun()) == nullptr) {
        interrupt->Idle(consoleON);
    }
...
Thread::Join()
{
    ASSERT(joinable);
  //  int msg;
  //  threadChannel->Receive(&msg); //Si el nodo tiene que comunicar su finalizacion... entonces bloqueo con un receive al
								  // padre que llama a hijo->join() hasta que el hijo informe por el canal que termino

	int* msg = new int;
	*msg = 0;

	threadChannel->Receive(msg);
	int rmsg = *msg;
	delete msg;

	///
	Thread* isEmp = scheduler->FindNextToRun();
	///
	 if(currentThread->tId == 0 && isEmp != nullptr && isEmp->tId != 0)
        currentThread->Yield();

	DEBUG('t', "Thread \"%s\" finished...\n", name);
	return rmsg;
}

void
Thread::SetInheritedPriority(Lock* lock, int newPriority)
{
	if(inheritedPriorities == nullptr) {
		inheritedPriorities = new PriorityInheritanceList;
		inheritedPriorities->lock = lock;
		inheritedPriorities->priority = newPriority;
		inheritedPriorities->next = nullptr;
		return;
	}
	...
	PriorityInheritanceList *newElement = new PriorityInheritanceList;
	newElement->lock = lock;
	newElement->priority = newPriority;
	newElement->next = inheritedPriorities;
	inheritedPriorities = newElement;
}
void
Thread::RemoveInheritedPriority(Lock* lock)
{
	for (PriorityInheritanceList* i = inheritedPriorities; i != nullptr; i = i->next) {
		if(i->lock == lock) {
			i->lock = nullptr;

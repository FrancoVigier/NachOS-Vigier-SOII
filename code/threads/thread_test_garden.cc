...
Turnstile(void *n_)
{
	...
	for (unsigned i = 0; i < ITERATIONS_PER_TURNSTILE; i++) {
		// No real parallelism, these operations are de facto atomic:
		int temp = count;
		count = temp + 1;
		// Parallelism is implemented with concurrency but without real parallelism.
		// Check if process/thread concurrency is implemented with only threads in
		//  this simulation. pthread() == fork()?
		currentThread->Yield();
	}
